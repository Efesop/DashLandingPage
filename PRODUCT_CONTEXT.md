...
- SEO pages: `/private-notes`, `/encrypted-notes`, `/offline-notes`, `/secure-journal`, and (Sep 8 2026, on `ArticleLayout`) `/best-notes-app-for-mac`, `/password-protected-notes`, `/obsidian-alternatives`, `/notion-alternatives`, `/evernote-alternatives`, `/privnote-alternatives`, `/guides/lock-notes-on-iphone-and-mac`, `/open-source-notes-app`, `/journal-app-mac`, `/day-one-alternative` — targeting and the build queue live in [SEO.md](./SEO.md). `/for-students`, `/encrypted-notes` and `/secure-journal` (now "Encrypted Diary" in the menus) moved onto `ArticleLayout` in Sep 2026
