...
const TOKEN_SECRET = process.env.DOWNLOAD_TOKEN_SECRET || 'your-super-secure-secret-key'
...
export async function verifyPaymentSession(sessionId: string): Promise<PaymentVerification> {
  try {
    if (!stripe) {
...
export function cleanupExpiredTokens(): void {
  ...
  for (const [tokenId, tokenData] of tokenStorage.entries()) {
